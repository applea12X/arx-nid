...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help='Number of packets per flow'
    )
    parser.add_argument(
        '--num-features',
        type=int,
    ...
    attack_types = ['ddos', 'port_scan', 'exfiltration'] if args.attack_type == 'all' else [args.attack_type]
    ...
    for attack_type in attack_types:
        ...
        metadata = {
            ...
            'statistics': {
                ...
                'total_bytes_orig': float(np.sum(flow[:, 0])) if flow.shape[1] > 0 else 0,
                'total_bytes_resp': float(np.sum(flow[:, 1])) if flow.shape[1] > 1 else 0,
        ...
        print(f"  Total resp bytes: {np.sum(flow[:, 1]):.0f}" if flow.shape[1] > 1 else "")
        print(f"  Avg duration: {np.mean(flow[:, 4]):.3f}s" if flow.shape[1] > 4 else "")
    ...
    for attack_type in attack_types:
        print(f"  python scripts/make_shap.py --sample synthetic --limit 1 --data-path data/processed/synthetic_{attack_type}.npy")
