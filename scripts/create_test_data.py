...
def generate_flow_data(num_flows=1000, duration_hours=1):
    ...
    services = ['http', 'https', 'ssh', 'dns', 'smtp', 'ftp', None]
    ...
    for _ in range(min(50, num_flows // 20)):  # ~5% of flows will be part of persistent connections
    ...
    for i in range(num_flows):
        ...
        if i == 0:
            ...
            if np.random.random() < 0.3:  # 30% chance of clustered timing
                ...
                ts = base_time.timestamp() + np.random.exponential(10) * i / num_flows * 3600
        
        # Generate connection tuple - sometimes reuse persistent connections
        if persistent_connections and np.random.random() < 0.4:  # 40% chance to use persistent connection
        ...
        if proto == 'icmp':
            ...
            conn_state = np.random.choice(conn_states, p=[0.8, 0.05, 0.05, 0.03, 0.05, 0.02])
        
        flow = {
            'ts': ts if isinstance(ts, float) else ts.timestamp(),
            'uid': f'C{i:06d}',
            'id.orig_h': orig_h,
            'id.orig_p': orig_p,
            'id.resp_h': resp_h,
...
def main():
    ...
    print(f"  Time range: {pd.to_datetime(df['ts'].min(), unit='s')} to {pd.to_datetime(df['ts'].max(), unit='s')}")
