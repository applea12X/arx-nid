...
def create_sliding_windows(df: pd.DataFrame, 
                          window_size: int = 20,
                          stride: int = 1,
                          min_flow_length: int = 5) -> np.ndarray:
    """
...
def save_tensor_data(tensor: np.ndarray, 
                    metadata: List[dict],
                    output_path: str,
...
def main():
    ...
    parser.add_argument("--input", default="data/processed/flows_v0.parquet",
                       help="Input Parquet file with preprocessed flows")
    parser.add_argument("--output", default="data/processed/tensor_v0.npy",
                       help="Output tensor file path")
    parser.add_argument("--metadata", default="data/processed/tensor_metadata_v0.parquet",
                       help="Output metadata file path")
    parser.add_argument("--window-size", type=int, default=20,
                       help="Sequence length for each sample")
    parser.add_argument("--stride", type=int, default=1,
                       help="Step size between windows")
    parser.add_argument("--min-flow-length", type=int, default=5,
                       help="Minimum flow length to include")
    ...
    print(f"Creating sequences (window_size={args.window_size}, stride={args.stride})...")
