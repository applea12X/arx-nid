...
def main():
    parser = argparse.ArgumentParser(description="Generate Integrated Gradients explanations")
    parser.add_argument(
        '--samples',
        type=int,
        default=1,
    ...
    parser.add_argument(
        '--output-dir',
        default='reports',
        help='Output directory for IG results'
    )
    parser.add_argument(
        '--data-path',
    ...
    print(f"Computing attributions for {args.samples} samples with {args.steps} steps...")
    ...
    print(f"  Attribution range: [{attributions_np.min():.6f}, {attributions_np.max():.6f}]")
