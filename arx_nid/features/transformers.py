    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        for stat in self.stats:
            for col in numeric_cols:
                ...
                if stat == "mean":
                    rolled = (
                        X_indexed.groupby(groupby_cols)[col]
                        .rolling(self.span)
                        .mean()
                        .reset_index()
                    )
        ...
        for rolled in rolled_features:
            ...
            result = result.merge(
                rolled[merge_cols + [rolled.columns[-1]]], on=merge_cols, how="left"
            )
...
class FlowFeatures(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        # Asymmetry metrics
        X["byte_asymmetry"] = (X["orig_bytes"] - X["resp_bytes"]) / (
            X["total_bytes"] + 1
        )
...
class CategoricalEncoder(BaseEstimator, TransformerMixin):
    ...
    def __init__(
        self, categorical_cols: List[str] = None, handle_unknown: str = "ignore"
    ):
        """
    ...
    def fit(self, X: pd.DataFrame, y=None):
        ...
        self.encoder = OneHotEncoder(
            handle_unknown=self.handle_unknown, sparse_output=False, dtype=np.float32
        )
...
class FlowPreprocessor:
    ...
    def __init__(
        ...
        self.rolling_transformer = RollingStats(
            span_s=rolling_window_s, stats=rolling_stats
        )
